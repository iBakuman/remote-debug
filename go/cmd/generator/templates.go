...
const dockerfileTpl = `FROM golang:{{.GoVersion}}-bullseye
ENV CGO_ENABLED 1
...
COPY dlv.sh /entrypoint.sh
RUN chmod +x /entrypoint.sh

ENV DELVE_PORT {{.DelvePort}}
...
const dockerComposeTpl = `version: '3'
services:
  debug:
    build: .
    environment:
...
const envTpl = `SRC_DIR={{.SrcDir}}
MAIN_FILE_PATH={{.MainFile}}
DELVE_PORT={{.DelvePort}}
`

const dlvScriptTpl = `#!/usr/bin/env bash
